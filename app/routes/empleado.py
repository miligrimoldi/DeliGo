...
def productos_servicio(id_servicio, id_categoria):
    ...
    return jsonify([{
        ...
        "tiempo_limite": p.tiempo_limite.strftime("%Y-%m-%d %H:%M:%S") if p.tiempo_limite else None,
    } for p in productos])
...
def opiniones_generales_servicio(id_servicio):
    ...
    data = [{
        ...
        "puntaje": op.puntaje,
        "fecha": op.fecha.strftime("%Y-%m-%d %H:%M:%S") if op.fecha else None
    } for op in opiniones]
...
def opiniones_por_pedido(id_pedido):
    ...
    return jsonify({
        ...
        "servicio": {
            ...
            "fecha": opinion_servicio.fecha.strftime("%Y-%m-%d %H:%M:%S") if opinion_servicio and opinion_servicio.fecha else None
        } if opinion_servicio else None,
        "productos": opiniones_productos
    })


# Second file - producto_servicio_usuario.py
from flask import Blueprint, jsonify, request
